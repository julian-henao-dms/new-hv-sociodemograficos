...
export class EstudiosFormComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    const institucion = await this.getAnyInformation(
      '/hojadevida/subcriterios/' + idEmp + '/' + numRegla + '/' + 'institucion'
    );
    this.instituciones = _.orderBy(institucion, ['id'], ['asc']);

    const titulo = await this.getAnyInformation(
      '/hojadevida/subcriterios/' + idEmp + '/' + numRegla + '/' + 'titulo'
    );
    this.titulos = _.orderBy(titulo, ['id'], ['asc']);

    const tipoCurso = await this.getAnyInformation(
      '/hojadevida/subcriterios/' + idEmp + '/' + numRegla + '/' + 'tipo_curso'
    );
    ...
    if (
      this.todosDatosCandidato.estudios &&
      this.todosDatosCandidato.estudios.length > 0
    ) {
