...
export class DatosAdicionalesFormComponent implements OnInit, OnChanges {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (experienciaEspecifica === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las opciones de años de experiencia'
      );
      return;
    }
    this.aniosExp = _.orderBy(experienciaEspecifica, ['id'], ['asc']);

    const aspiracionSalarial = await this.getAnyInformation(
    ...
    if (aspiracionSalarial === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar los rangos salariales'
      );
      return;
    }
    this.salarios = _.orderBy(aspiracionSalarial, ['id'], ['asc']);

    const fuenteReclutamiento = await this.getAnyInformation(
      '/hojadevida/subcriterios/' +
        idEmp +
        '/' +
        numRegla +
    ...
    if (fuenteReclutamiento === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las fuentes de reclutamiento'
      );
      return;
    }
    this.fuentesReclutamiento = _.orderBy(fuenteReclutamiento, ['id'], ['asc']);

    const entidadExpedicion = await this.getAnyInformation(
      '/hojadevida/subcriterios/' + idEmp + '/' + numRegla + '/' + 'entidad'
    );
    if (entidadExpedicion === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las entidades de expedición'
      );
      return;
    }
    this.entidades = _.orderBy(entidadExpedicion, ['id'], ['asc']);

    const eps = await this.getAnyInformation(
      '/hojadevida/subcriterios/' + idEmp + '/' + numRegla + '/' + 'eps'
    );
    if (eps === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las EPS'
      );
      return;
    }
    this.epss = _.orderBy(eps, ['id'], ['asc']);

    const fondoPension = await this.getAnyInformation(
      '/hojadevida/subcriterios/' +
        idEmp +
        '/' +
        numRegla +
    ...
    if (fondoPension === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar los fondos de pensión'
      );
      return;
    }
    this.fondosPension = _.orderBy(fondoPension, ['id'], ['asc']);

    const cajaCompensacion = await this.getAnyInformation(
      '/hojadevida/subcriterios/' + idEmp + '/' + numRegla + '/' + 'fondo_caja'
    );
    if (cajaCompensacion === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las cajas de compensación'
      );
      return;
    }
    this.cajasCompensacion = _.orderBy(cajaCompensacion, ['id'], ['asc']);

    const cesantia = await this.getAnyInformation(
      '/hojadevida/subcriterios/' +
        idEmp +
        '/' +
        numRegla +
    ...
    if (cesantia === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las cesantias'
      );
      return;
    }
    this.cesantias = _.orderBy(cesantia, ['id'], ['asc']);

    const categoriaLicencia = await this.getAnyInformation(
      '/hojadevida/subcriterios/' +
        idEmp +
        '/' +
        numRegla +
    ...
    if (categoriaLicencia === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las categorías de licencia'
      );
      return;
    }
    this.categoriasLicencia = _.orderBy(categoriaLicencia, ['id'], ['asc']);

    const grupoSanguineo = await this.getAnyInformation(
      '/hojadevida/subcriterios/' +
        idEmp +
        '/' +
        numRegla +
    ...
    if (grupoSanguineo === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las opciones de grupo sanguineo'
      );
      return;
    }
    this.gruposSanguineos = _.orderBy(grupoSanguineo, ['id'], ['asc']);

    const colorPiel = await this.getAnyInformation(
      '/hojadevida/subcriterios/' + idEmp + '/' + numRegla + '/' + 'color_piel'
    );
    if (colorPiel === null) {
      this.messageService.error(
        'Error',
        'Error interno del servidor al cargar las opciones de color de piel'
      );
      return;
    }
    ...
    this.todosCandidatoStorage.candidato.id_rh_experiencia_equipo = this
      .todosCandidatoStorage.candidato.id_rh_experiencia_equipo
  ...
  public guardarProgreso() {
    if (!this.fieldDatosAdicionales.valid) {
      ...
      this.todosCandidatoStorage.categorias = [...this.myReferenceArray];
      this.todosCandidatoStorage.candidato.id_Entidad == null
        ? (this.todosCandidatoStorage.candidato.id_Entidad = 0)
        : this.todosCandidatoStorage.candidato.id_Entidad;
